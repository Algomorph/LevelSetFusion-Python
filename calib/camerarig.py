...
class CameraRig(object):
    ...
    def to_xml(self, root_element, as_sequence=False):
        ...
        cameras_elem = etree.SubElement(rig_elem, CameraRig.CAMERAS_ELEMENT_TAG)
        if self.extrinsics is not None:
    ...
    def from_xml(element):
        """
        Build a Rig object out of the given xml node
        ...
        if use_old_format:
            ...
            cameras_elem = element.find(CameraRig.CAMERAS_ELEMENT_TAG)
            ...
            cameras_element = element.find(CameraRig.CAMERAS_ELEMENT_TAG)
            ...
            if extrinsics_element is not None:
                ...
                extrinsics = None
        return CameraRig(cameras, extrinsics)


class DepthCameraRig(CameraRig):
    ...
    def __init__(self, cameras=(), extrinsics=None):
        ...
        first camera in the tuple.
        """
        super().__init__(cameras, extrinsics)
        self.depth_cameras = []
        self.color_cameras = []
        self.depth_camera = None
        self.color_camera = None
        for camera in self.cameras:
            if type(camera) == DepthCamera:
                if self.depth_camera is None:
                    self.depth_camera = camera
                self.depth_cameras.append(camera)
            elif type(camera) == Camera:
                if self.color_camera is None:
                    self.color_camera = camera
                self.color_cameras.append(camera)
        if self.depth_camera is None:
            raise ValueError("Expected at least a single object of type 'DepthCamera' in the cameras array")
    ...
    def from_infinitam_format(file_handle_or_name):
        ...
        color_intrinsics_lines = [file_handle.readline(), file_handle.readline()]
        file_handle.readline()
        depth_resolution_line = file_handle.readline()
        depth_intrinsics_lines = [file_handle.readline(), file_handle.readline()]
        file_handle.readline()
        extrinsics_lines = [file_handle.readline(), file_handle.readline(), file_handle.readline()]
        file_handle.readline()
        depth_ratio_and_offset_line = file_handle.readline()
        color_camera = Camera.from_infinitam_format(color_resolution_line, color_intrinsics_lines)
        depth_camera = DepthCamera.from_infinitam_format(depth_resolution_line, depth_intrinsics_lines,
                                                         extrinsics_lines, depth_ratio_and_offset_line)
...
class MultiStereoRig(object):
    ...
    """
    RIGS_ELEMENT_TAG = CameraRig.__name__ + "s"
    ...
    def __init__(self, stereo_rigs=None, rig=None):
        ...
        @type stereo_rigs: tuple[calib.rig.CameraRig]
        ...
        if stereo_rigs is not None and rig is None:
            ...
            rigs = [CameraRig(cameras=(rig.cameras[0], rig.cameras[1], Camera.Extrinsics()))]
            ...
            for ix_cam in range(2, len(rig.cameras), 2):
                ...
                rigs.append(CameraRig(cameras=(left_cam, right_cam), extrinsics=stereo_rig_extrinsics))
    ...
    def from_xml(element):
        ...
        rigs = tuple([CameraRig.from_xml(rig_element) for rig_element in rigs_element])
