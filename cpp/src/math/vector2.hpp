...
public:
	...
	_CPU_AND_GPU_CODE_ explicit Vector2(const Vector2_<T> &v) { this->x = v.x; this->y = v.y; }// copy constructor
	...
	_CPU_AND_GPU_CODE_ inline Vector2<unsigned char> to_uchar() const {
		Vector2<int> vi = to_int(); return {(unsigned char)CLAMP(vi.x, 0, 255), (unsigned char)CLAMP(vi.y, 0, 255)};
	}
	...
	// type-cast operators
	_CPU_AND_GPU_CODE_ explicit operator T *() { return this->values; }
	_CPU_AND_GPU_CODE_ explicit operator const T *() const { return this->values; }
